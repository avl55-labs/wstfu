...
|:-------:|-----|------------|-------------|
| 1 | `mute` | Только контроль перезагрузки. Обновления качаются и ставятся. | Ожидающая перезагрузка ходит за тобой, пока не сделаешь сам. |
...
| 3 | `stfu` **(по умолчанию)** | `quiet` + скользящая пауза: ничего не прилетает, пока не откроешь окно. | Патчей безопасности нет, пока не откроешь окно — ок для рабочей станции, не для разъездного ноутбука. |
...
> потому что именно подпись говорит Windows, кому доверять, а её нет. Чтобы прога
> запустилась и осталась на месте, внеси в исключения Defender две папки:
> **`C:\ProgramData\WSTFU`** (сюда ставится и отсюда работает) и **папку, из
> которой запускаешь `wstfu.ps1`**. `shutup` пытается добавить первую сам — см.
> [Defender](#defender), если не вышло.

Нужны **Windows 10 / 11 Pro, Enterprise, Education или IoT LTSC** и PowerShell от
администратора. (Home эти политики игнорирует.)
...
Любишь кнопки? Двойной клик по **`Start WSTFU.cmd`** — откроется панель
управления. `status.cmd`, `shutup.cmd` и `speak.cmd` тоже на месте — они сами
просят повышение прав.
